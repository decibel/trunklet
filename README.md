...
installed, you can upgrade it to a properly packaged extension with:
    ...
    CREATE EXTENSION trunklet FROM unpackaged;

For versions of PostgreSQL less than 9.1.0, you'll need to run the
installation script:

    psql -d mydb -f /path/to/pgsql/share/contrib/trunklet.sql

If you want to install trunklet and all of its supporting objects into a specific
schema, use the `PGOPTIONS` environment variable to specify the schema, like
so:

    PGOPTIONS=--search_path=extensions psql -d mydb -f trunklet.sql

Dependencies
------------
The `trunklet` data type has no dependencies other than PostgreSQL.
